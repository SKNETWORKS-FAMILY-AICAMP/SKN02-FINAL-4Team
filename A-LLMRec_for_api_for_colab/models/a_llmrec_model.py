...
class A_llmrec_model(nn.Module):
    def __init__(self, args):
        ...
        if args.pretrain_stage2 or args.inference:
            ...
            vectorstore = Chroma(persist_directory="./chroma_database", embedding_function=OpenAIEmbeddings(model='text-embedding-3-small'))
    ...
    def forward(self, data, optimizer=None, batch_iter=None, mode='phase1'):
        ...
        if mode == 'phase2':
            # 수정된 부분: 손실을 반환하도록 변경
            loss = self.pre_train_phase2(data)
            return loss
        if mode =='generate':
            self.generate(data)
    ...
    def pre_train_phase2(self, data):
        u, seq, pos, neg = data
        mean_loss = 0
        ...
        # 출력 부분은 여기서 처리하지 않음
        return loss_rm  # 손실 반환
    
    def make_candidate_text_rag(self, user_query, candidate_num):
        # RAG를 사용하여 후보 아이템을 가져옵니다.
        candidate_items = self.retrieve_candidates_rag(user_query, candidate_num)
        ...
        for item in candidate_items:
            ...
            candidate_text.append(item_title + '[CandidateEmb]')
            candidate_ids.append(item_id)

        # 후보 아이템을 무작위로 섞기
        random_indices = np.random.permutation(len(candidate_text))
        candidate_text = np.array(candidate_text)[random_indices]
        candidate_ids = np.array(candidate_ids)[random_indices]
        print (candidate_text,candidate_ids)
        return ','.join(candidate_text), candidate_ids
    


    def retrieve_candidates_rag(self, user_query, candidate_num):
        ...
        for doc in docs:
            ...
            item_title = doc.page_content

            # 이미 상호작용한 아이템은 제외 (필요한 경우)
            # if item_id in interact_ids:
            #     continue
            candidate_items.append({'item_id': item_id, 'item_title': item_title})

            if len(candidate_items) >= candidate_num:
                break
    ...
    def generate(self, data):
        ...
        with torch.no_grad():
            ...
            # 디버깅 코드: input_ids가 비어있는지 확인
            if llm_tokens.input_ids.nelement() == 0:
                print("Warning: llm_tokenizer input_ids is empty!")
            ...
            with torch.cuda.amp.autocast():
                ...
                attention_mask = torch.cat([atts_llm, llm_tokens.attention_mask], dim=1)
                    ...
                    ## 수정된 부분 num_beam 1 -> 10 , num_return_sequences=1 -> 10
                ...
                outputs = self.llm.llm_model.generate(
                    ...
                    temperature=1,
                    num_beams=5,
                    max_new_tokens=50,  # max_new_tokens로 설정
                    min_length=1,
    ...
    def generate2(self, data, user_query):
        u, seq, pos, neg, rank = data
        ...
        with torch.no_grad():
            ...
            for i in range(len(u)):
                ...
                interact_text, interact_ids = self.make_interact_text(seq[i][seq[i]>0], 10)
                candidate_num = 50
                candidate_text, candidate_ids = self.make_candidate_text_rag(user_query,candidate_num)

            

                input_text = ''
                input_text += 'This user has previously purchased the following items: '
                input_text += interact_text
                input_text += '. Based on this purchase history, please recommend multiple items that this user might want to buy next from the following candidate list: '
                input_text += candidate_text
                input_text += '. The recommended items are: '
        ...
        with torch.no_grad():
            ...
            with torch.cuda.amp.autocast():
                ...
                outputs = self.llm.llm_model.generate(
                    ...
                    temperature=0.3,
                    num_beams=1,
                    max_new_tokens=200,  # max_new_tokens로 설정
                    min_length=35,
                    pad_token_id=self.llm.llm_tokenizer.eos_token_id,
            ...
            output_text = self.llm.llm_tokenizer.batch_decode(outputs, skip_special_tokens=True)
            output_text = [text.strip() for text in output_text]
            result = "model's recommendation :",output_text,"\ncandidate 50 :",candidate_text
