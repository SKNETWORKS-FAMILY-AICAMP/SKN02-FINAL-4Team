...
def train_model_phase1_(rank, world_size, args):
    ...
    # Dataloader, Model, Optimizer 초기화
...
def train_model_phase2_(rank, world_size, args):
    ...
    # 시작 에포크를 추가로 받아서 설정
    start_epoch = args.start_epoch if hasattr(args, 'start_epoch') else 1

    model = A_llmrec_model(args).to(args.device)
    phase1_epoch = args.phase1_epoch  # 스테이지 1에서 로드할 에포크 번호
    phase2_epoch = start_epoch - 1 if start_epoch > 1 else None
    ...
    # 진행률 표시를 강화하여 보다 상세한 정보 출력
    for epoch in range(start_epoch, args.num_epochs + 1):
        if args.multi_gpu:
            train_data_loader.sampler.set_epoch(epoch)
        
        progress_bar = tqdm(train_data_loader, desc=f"Epoch {epoch}/{args.num_epochs}", 
                            total=len(train_data_loader), 
                            bar_format="{l_bar}{bar:10}{r_bar}{bar:-10b}")  # bar_format을 추가하여 더 상세한 정보 제공
        
        for step, data in enumerate(progress_bar):
            u, seq, pos, neg = data
            u, seq, pos, neg = u.numpy(), seq.numpy(), pos.numpy(), neg.numpy()
            # 모델의 forward 및 손실 계산
            loss = model([u, seq, pos, neg], mode='phase2')
            loss = loss / args.accumulation_steps  # 손실 스케일링
            loss.backward()
            
            # Gradient Accumulation 조건에 따른 옵티마이저 스텝
            if (step + 1) % args.accumulation_steps == 0:
                adam_optimizer.step()
                adam_optimizer.zero_grad()

            # 진행률 바에 loss 및 다른 정보를 표시
...
def inference_(rank, world_size, args):
    ...
    for _, data in tqdm(enumerate(inference_data_loader), total=len(inference_data_loader), desc="Inference Progress"):
        u, seq, pos, neg = data
        u, seq, pos, neg = u.numpy(), seq.numpy(), pos.numpy(), neg.numpy()
